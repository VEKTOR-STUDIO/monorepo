      ...
      <div className="max-w-7xl mx-auto px-8 relative z-10">
        <div className="text-center mb-16">
          ...
          <h2 ref={titleRef} className="font-extrabold text-4xl md:text-5xl tracking-tight mb-6 text-base-content">
            {renderSegment("Marcas que", "seg1")}{" "}
            <span className="text-accent">{renderSegment("confían en esta plantilla", "seg2")}</span>
          </h2>
          <p ref={subtitleRef} className="text-lg text-base-content/80 max-w-2xl mx-auto">
            Testimonios de ejemplo para tu nicho. Sustituye por casos reales cuando tengas autorización
            y fotografía acorde a tu marca.
          </p>
        </div>

        <div ref={cardsRef} className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          <Testimonial
            quote="Esta herramienta ha revolucionado la forma en que operamos nuestro negocio."
            author="Nombre del Cliente"
            role="Cargo o profesión"
          />
          <Testimonial
            quote="Describe aquí el resultado y la experiencia que quieres destacar para futuros clientes."
            author="Nombre del Cliente"
            role="Cargo o profesión"
          />
          <Testimonial
            quote="Texto genérico de confianza: agenda, comunicación y resultados alineados con lo prometido."
            author="Nombre del Cliente"
            role="Cargo o profesión"
          />
        </div>
        ...
        <div ref={founderRef} className="text-center bg-base-100/60 backdrop-blur-md rounded-2xl p-8 border border-base-content/20">
          <p className="text-sm text-base-content/60 mb-4 uppercase tracking-widest">Tu marca aquí</p>
          <div className="flex flex-col items-center gap-2">
            <p className="font-black text-2xl text-base-content">{config.appName}</p>
            <p className="text-base-content/80 max-w-2xl leading-relaxed">
              {config.business?.tagline ?? "Personaliza este bloque con la propuesta de tu estudio."}
            </p>
          </div>
        </div>
      </div>
