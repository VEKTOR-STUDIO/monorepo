...
// la página: es un argumento para el dueño, no para quien viene a ver camionetas.
    ...
    >
      <nav className="mx-auto flex h-16 max-w-7xl items-center gap-6 px-4 sm:px-6">
        ...
        <button
          ...
          className="ml-auto rounded-lg p-2 text-base-content/70 transition-colors hover:bg-base-300 md:hidden"
          ...
          <div className="flex flex-col gap-1">
            {(esDemo() ? [...ENLACES, ENLACE_DEMO] : ENLACES).map((enlace) => (
              <Link
                key={enlace.href}
                href={enlace.href}
                className="rounded-lg px-3 py-2.5 text-base font-medium text-base-content/80 transition-colors hover:bg-base-300"
              >
                {enlace.texto}
              </Link>
            ))}
