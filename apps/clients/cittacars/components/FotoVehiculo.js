        ...
        <div className="absolute inset-0 flex items-center justify-center p-[8%]">
          <div className="relative w-full max-w-2xl">
            <Silueta
              ...
              className={`w-full ${tono === "oscuro" ? "text-base-content/20" : "text-base-content/25"}`}
            />
...
export function AvisoSinFoto({ className = "" }) {
  return (
    <span
      className={`cifra rounded-lg bg-base-content/8 px-2.5 py-1 text-[0.65rem] text-base-content/45 ${className}`}
    >
      Foto pendiente
    </span>
  );
...
export function AvisoFotoStock({ className = "" }) {
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-lg bg-base-content/8 px-3 py-1.5 text-[0.7rem] text-base-content/55 ${className}`}
    >
