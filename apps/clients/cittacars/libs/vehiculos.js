...
// El inventario.
//
// La fuente de verdad es data/vehiculos.json, que replica cómo publica el
// concesionario: un vehículo por publicación, con su ficha y su texto. Se lee
// del disco, no de una base de datos, porque un inventario de decenas de
 ...
 *
 * Citta Cars no vende motos —su feed es de camionetas, pick-up y carros—, así
 * que aquí hay tres y no cuatro. Si algún día entra otra carrocería, se añade
 * con su foto en public/landing/tipo-<slug>.jpg y el resto se ajusta solo.
 */
export const TIPOS = [
...
/* Los tonos son los claros de cada color: sobre el negro de la página, un
   emerald-700 no se lee. */
...
/** A dónde cae una carrocería que no esté en la lista. */
const SEDAN = TIPOS.find((t) => t.slug === "sedan");
...
function normalizar(v) {
  return {
    ...
    tipo: TIPOS.find((t) => t.slug === v.carroceria) || SEDAN,
 ...
 */
export function filtrar(vehiculos, { tipo, marca, precioMax, anioMin, q, orden } = {}) {
  let lista = [...vehiculos];
