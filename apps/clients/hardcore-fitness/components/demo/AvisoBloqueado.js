 ...
 *
 * Aparece donde una acción está desactivada: el envío del pedido, las acciones
 * del panel. Explica qué haría el sistema de verdad, porque decir solo
 * "desactivado" no vende nada.
 */
export default function AvisoBloqueado({ titulo, children, className = "" }) {
  return (
    <div
      className={`rounded-xl border border-primary/35 bg-primary/8 p-4 ${className}`}
      role="status"
    >
