  ...
  return (
    <header
      className={`fixed top-0 w-full z-40 transition-all duration-300 ${
        scrolled ? "bg-base-100 border-b border-base-300 shadow-sm" : "bg-transparent"
      }`}
    >
      <nav className="container flex items-center justify-between px-6 sm:px-8 py-4 mx-auto" aria-label="Global">
        <div className="flex lg:flex-1 relative z-50">
          <Link className="flex items-center gap-2 shrink-0 group" href="/" title={`${config.appName} — Inicio`}>
      ...
      <div className={`fixed inset-0 z-50 ${isOpen ? "" : "hidden"}`}>
        ...
        />
        <div
          className={`fixed inset-y-0 right-0 z-50 w-full px-8 py-6 overflow-y-auto bg-base-100 border-l border-base-300 shadow-sm ${
        ...
        >
          ...
          <div className="flex flex-col gap-6">
            {links.map((link) => (
              <Link
                href={link.href}
                key={link.href}
                className="text-base-content/80 hover:text-primary transition-colors text-sm font-medium border-b border-base-200 pb-4"
                title={link.label}
                onClick={() => setIsOpen(false)}
              >
                {link.label}
          ...
          <div className="mt-10 flex flex-col gap-3">
            <div className="flex items-center gap-3">
              <span className="text-base-content/50 text-xs uppercase tracking-wider">Tema</span>
              <ThemeSwitch />
            </div>
            {cta}
          </div>
