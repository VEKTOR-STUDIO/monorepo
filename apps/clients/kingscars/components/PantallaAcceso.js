 ...
 *
 * Aquí manda la firma de Vektor y no la marca de Kings Cars: quien
 * llega todavía no es cliente de Kings Cars, es alguien a quien se le está
...
export default function PantallaAcceso({ destino = "/" }) {
  ...
  return (
    <main ref={raiz} className="puerta relative flex h-svh flex-col overflow-hidden bg-base-100">
      ...
      {/* Velo. El escenario de la portada lleva la corona al agua a tamaño
          completo, y justo ahí cae el rótulo: blanco sobre blanco, aunque el
          blanco de abajo esté al 5 %. El velo baja el fondo lo justo para que
          la corona se siga adivinando y el texto se lea encima. */}
      ...
      <div
        ...
        aria-hidden="true"
        // El halo va en BLANCO y no en rojo, aunque el rojo fuera lo llamativo:
        // esta marca usa el rojo para señalar tres cosas contadas, y un halo de
        // 40 rem teñido de rojo baña la pantalla entera con el único color que
        // aquí significa algo. En blanco hace lo que tiene que hacer —levantar
        // el centro— sin gastarse el acento.
        style={{
          background:
            "radial-gradient(circle, color-mix(in oklab, var(--color-hueso) 16%, transparent), transparent 62%)",
          filter: "blur(90px)",
        }}
      />
        ...
        <div className="m-auto grid w-full max-w-6xl gap-x-[clamp(1.5rem,4vw,3.5rem)] gap-y-[var(--puerta-v4)] px-5 py-[var(--puerta-v2)] lg:grid-cols-[1.15fr_1fr]">
          ...
          <div className="flex min-w-0 flex-col items-center text-center lg:items-start lg:text-left">
            ...
            <div data-puerta="logo" className="mt-[var(--puerta-v3)] flex flex-col items-center lg:items-start">
              ...
              {/* Solo el emblema, sin la corona en vector al lado: los dos
                  dibujan la misma corona y puestos juntos parecían dos logos de
                  dos negocios distintos. */}
              <div className="mt-[var(--puerta-v1)] flex items-center gap-4">
                <LogoChapa lado="var(--puerta-logo)" prioridad />
              </div>
              <h1 className="display-abierto mt-[var(--puerta-v2)] text-[clamp(1.6rem,4.4vh,3rem)]">
                Kings Cars
              </h1>
            </div>
            ...
            <p data-puerta="titulo" className="puerta-presentacion mt-[var(--puerta-v3)] max-w-md leading-relaxed text-sm text-base-content/65">
              ...
              separados, tu página de «publica tu vehículo gratis», y la ficha de tus
              propios posts —el rótulo, el kilometraje, la transmisión y el precio—.
              Entra con tu contraseña y recórrela.
            </p>
