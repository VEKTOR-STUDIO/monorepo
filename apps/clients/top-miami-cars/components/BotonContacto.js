 ...
 *
 * Fuera de demo abre WhatsApp con la unidad ya escrita en el mensaje. Es el
 * único paso que de verdad importa en este negocio: el comprador escribe
 * sabiendo qué pregunta, y quien contesta sabe de qué unidad le hablan.
 *
 * En demo NO abre nada. No es un adorno desactivado: el número es el real de
 * su ficha de Google, así que si abriera, cualquiera que esté probando la demo
 * le estaría mandando mensajes falsos al negocio. En su lugar explica qué haría
...
export default function BotonContacto({
  ...
  if (demo) {
    return (
      <button
        ...
        onClick={() =>
          toast(
            `En la página entregada, esto abre tu WhatsApp${
              sede ? ` (${sede.telefono})` : ""
  ...
  // Sin WhatsApp ni Instagram no hay a dónde mandar a nadie: mejor no pintar
  // un botón que un enlace vacío.
