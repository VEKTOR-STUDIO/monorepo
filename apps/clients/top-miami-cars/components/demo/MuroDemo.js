...
export default function MuroDemo({ ocultos, children }) {
  return (
    <div className="relative mt-5">
      ...
      <div
        className="pointer-events-none max-h-104 select-none overflow-hidden blur-[7px] saturate-50 opacity-55"
        aria-hidden="true"
      >
        {children}
      </div>
      ...
      <div className="absolute inset-x-0 top-6 flex justify-center px-4">
        <div className="ficha resplandor w-full max-w-md p-7 text-center">
          <p className="microgramma text-[0.65rem] text-primary">Hasta aquí llega la demo</p>

          <p className="display mt-4 text-xl text-primary">
            <span>+{ocultos}</span>{" "}
            {ocultos === 1 ? "unidad más" : "unidades más"}
          </p>

          <p className="mt-3 text-sm leading-relaxed text-base-content/70">
            La página completa lleva todo el inventario, la ficha de cada unidad, los
            filtros por carrocería, marca y precio, y el botón de WhatsApp que te llega
            con el modelo ya escrito.
          </p>

          <p className="mt-5 flex items-baseline justify-center gap-2.5">
            {config.demo.precioAnterior && (
              <span className="cifra text-lg text-base-content/45 line-through">
                {config.demo.precioAnterior}
              </span>
            )}
            <span className="display text-3xl text-primary">{config.demo.precio}</span>
          </p>
          <p className="mt-1.5 text-xs text-base-content/55">{config.demo.precioNota}</p>
