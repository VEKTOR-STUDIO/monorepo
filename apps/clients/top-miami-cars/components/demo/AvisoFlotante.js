...
const LLAVE = "top-miami-cars.demo.aviso-cerrado";
...
export default function AvisoFlotante() {
  ...
  return (
    <div
      ...
      aria-hidden={!visible}
    >
      <div className="ficha resplandor pointer-events-auto w-full max-w-sm p-5">
        <button
          type="button"
          onClick={cerrar}
          className="absolute right-2.5 top-2.5 p-1.5 text-base-content/50 transition-colors hover:bg-base-300 hover:text-base-content"
        ...
        <p className="mt-2.5 pr-5 text-sm leading-relaxed text-base-content/70">
          Tu salón convertido en página: inventario con filtros, ficha por unidad y
          WhatsApp con el modelo ya escrito. Con tu azul, tu escudo y tu letra.
        </p>
          ...
          <div>
            <p className="flex items-baseline gap-2">
              {config.demo.precioAnterior && (
                <span className="cifra text-sm text-base-content/50 line-through">
                  {config.demo.precioAnterior}
                </span>
              )}
              <span className="display text-xl text-primary">{config.demo.precio}</span>
            </p>
            <p className="text-[0.7rem] text-base-content/55">{config.demo.precioNota}</p>
            <p className="mt-1 flex items-center gap-1.5">
              <span className="microgramma text-[0.55rem] text-base-content/55">quedan</span>
