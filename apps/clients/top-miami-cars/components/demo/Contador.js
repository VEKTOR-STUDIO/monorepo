  ...
  return (
    <div className={`flex gap-2 ${className}`}>
      {cajas.map((caja) => (
        <div
          key={caja.etiqueta}
          className="min-w-14 rounded-md border border-base-content/15 bg-base-100 px-2 py-2 text-center"
        >
          <p className="cifra text-xl font-bold tabular-nums text-primary">
            {dosCifras(caja.valor)}
          </p>
          <p className="microgramma mt-0.5 text-[0.55rem] text-base-content/55">
            {caja.etiqueta}
          </p>
        </div>
      ))}
