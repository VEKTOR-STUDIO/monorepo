...
export async function generateMetadata({ params }) {
  ...
  return getSEOTags({
    ...
    description: `${vehiculo.tituloLargo} ${vehiculo.anio}, ${vehiculo.km.toLocaleString("es-VE")} km, ${vehiculo.motor}. ${enDolares(vehiculo.precio)} en ${vehiculo.ubicacion}.`,
    ...
    <>
      ...
      <main>
        ...
        <div className="mx-auto max-w-7xl px-4 pt-6 sm:px-6">
          <Link
            href="/vehiculos"
            className="text-sm text-base-content/55 transition-colors hover:text-primary"
          >
        ...
        <article className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
          <div className="grid gap-10 lg:grid-cols-[1.35fr_1fr] lg:gap-14">
            ...
            <div>
              <div className="vidrio overflow-hidden p-1.5">
                <FotoVehiculo
                  vehiculo={vehiculo}
                  prioridad
                  className="aspect-4/3 w-full rounded-[0.7rem]"
              ...
              {vehiculo.fotos?.length > 1 && (
                <div className="mt-3 grid grid-cols-4 gap-3">
                  {vehiculo.fotos.slice(1, 5).map((foto) => (
                    <div key={foto} className="vidrio overflow-hidden p-1">
                      <FotoVehiculo
                        vehiculo={{ ...vehiculo, fotos: [foto] }}
                        className="aspect-4/3 w-full rounded-[0.6rem]"
                        sizes="20vw"
                      />
                    </div>
                  ))}
            ...
            <div className="lg:pt-2">
              <div className="flex flex-wrap items-center gap-3">
                ...
                {vehiculo.origen && (
                  <>
                    <span className="text-sm text-base-content/40">·</span>
                    <span
                      className={`cifra rounded-xl px-2.5 py-1 text-[0.65rem] font-semibold uppercase tracking-wider ${
                        vehiculo.origen === "Importado"
                          ? "bg-secondary/25 text-secondary-content"
                          : "panel text-base-content/60"
                      }`}
                    >
                      {vehiculo.origen}
                    </span>
              ...
              <div className="vidrio mt-7 px-6 py-5">
          ...
          <div className="mt-16 grid gap-12 border-t border-white/10 pt-12 lg:grid-cols-2 lg:gap-20">
            <div>
        ...
        {parecidos.length > 0 && (
          <section className="relative overflow-hidden border-t border-white/10 bg-base-200/40 px-4 py-20 sm:px-6">
