...
export default function AvisoBloqueado({ titulo, children, className = "" }) {
  return (
    <div
      className={`rounded-xl border border-primary/35 bg-primary/8 p-4 backdrop-blur-md ${className}`}
      role="status"
    >
      <p className="rotulo text-primary/80">{titulo || "Desactivado en la demo"}</p>
