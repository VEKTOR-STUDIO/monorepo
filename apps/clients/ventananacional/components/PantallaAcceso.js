 ...
 *
 * No es una pantalla de login: es el escaparate donde se vende el sistema, y la
 * única pantalla que ve seguro quien recibe el enlace —puede no tener la clave a
 * mano, puede perderla, puede mirar desde el móvil—. Por eso está montada como
 * la carátula de una edición: insignia, lo que trae, rebaja, cuenta atrás y
 * cintas de texto corriendo, con la caja de acceso a la derecha, que es donde la
 * vista termina cayendo.
 *
 * Todo lo que anuncia es verdad y está en `config.demo`: el precio, el precio de
 * antes, la fecha en que se acaba y la lista de lo que se lleva quien compre. Si
 * se vacía `precioAnterior`, el sello de rebaja desaparece solo.
 *
 * Aquí manda la firma de Vektor y no la marca de Venta Nacional: quien
 * llega todavía no es cliente de la concesionaria, es alguien a quien se le está
...
export default function PantallaAcceso({ destino = "/" }) {
        ...
        linea
          ...
          .from(
            "[data-puerta='logo']",
            {
              ...
              duration: 1,
              // Sin esto, GSAP deja puestos en línea el filtro y el transform al
              // terminar, y cualquiera de los dos crea un contexto de apilado
              // que se come el desenfoque del marco de cristal.
  ...
  // Al terminar la entrada, el cursor ya está en el campo. La animación no
  // bloquea: quien sabe la clave puede escribirla desde el primer momento.
  // Solo en escritorio: en el móvil la caja queda debajo de la lista de lo que
  // trae la edición, y el foco desplazaría la puerta hasta el formulario
  // saltándose el logotipo y lo que se vende (y en Android abriría el teclado).
  useEffect(() => {
  ...
  return (
    <main ref={raiz} className="puerta relative flex h-svh flex-col overflow-hidden">
      ...
      />
      <div className="absolute inset-0 bg-base-100/74" aria-hidden="true" />
      ...
      <div
        data-puerta="halo"
        className="pointer-events-none absolute left-1/2 top-1/3 size-[40rem] -translate-x-1/2 -translate-y-1/2 opacity-70"
        aria-hidden="true"
        style={{
          background:
            "radial-gradient(circle, color-mix(in oklab, var(--color-primary) 28%, transparent), transparent 62%)",
          filter: "blur(90px)",
        }}
      />
      ...
      <div className="relative z-10 flex min-h-0 flex-1 overflow-y-auto overflow-x-hidden overscroll-contain">
        <div className="m-auto grid w-full max-w-6xl gap-x-[clamp(1.5rem,4vw,3.5rem)] gap-y-[var(--puerta-v4)] px-5 py-[var(--puerta-v2)] lg:grid-cols-[1.15fr_1fr]">
          ...
          <div className="flex min-w-0 flex-col items-center text-center lg:items-start lg:text-left">
            <div data-puerta="marca" className="flex flex-wrap items-center justify-center gap-x-2 gap-y-1 sm:gap-x-3 lg:justify-start">
              <a
                ...
                className="microgramma text-xs text-base-content/80 transition-colors hover:text-primary"
                style={{ fontFamily: "var(--microgramma-font)", letterSpacing: "0.22em" }}
              >
                Alessandrovaru
              </a>
              <span className="microgramma text-[0.6rem] text-base-content/45">de</span>
              <a
              ...
              <span className="flex items-center gap-2 sm:gap-3">
                <span className="h-px w-4 bg-white/25 sm:w-8" aria-hidden="true" />
            ...
            <div data-puerta="logo" className="mt-[var(--puerta-v3)] flex flex-col items-center lg:items-start">
              ...
              {/* En el móvil la foto y el nombre van uno al lado del otro: apilados
                  son 150 px de marca antes de llegar al precio. De `lg` en
                  adelante, donde hay dos columnas, vuelve la carátula apilada. */}
              <div className="mt-[var(--puerta-v1)] flex items-center gap-4 lg:mt-0 lg:flex-col lg:items-start lg:gap-0">
                <LogoFoto lado="var(--puerta-logo)" className="lg:mt-[var(--puerta-v1)]" prioridad />
                <div className="text-left">
                  <h1 className="display text-[clamp(1.6rem,4.4vh,3rem)] lg:mt-[var(--puerta-v2)]">
                    <span className="cromo">Venta Nacional</span>
                  </h1>
                  <p className="puerta-cortesia cifra mt-1 text-[0.6rem] uppercase tracking-[0.24em] text-base-content/40">
            ...
            <p data-puerta="titulo" className="puerta-presentacion mt-[var(--puerta-v3)] max-w-md leading-relaxed text-sm text-base-content/65">
              La página de la concesionaria, entera y funcionando: inventario, fichas,
              filtros y las dos salas. Entra con tu contraseña y recórrela; lo que ves
            ...
            <div className="puerta-incluido mt-[var(--puerta-v3)] w-full">
              ...
              <ul data-puerta="dlc" className="puerta-incluye mt-[var(--puerta-v2)]">
                {incluye.map((cosa, i) => (
                  <li
                    key={cosa}
                    className="linea-dlc flex items-start gap-3 rounded-xl px-3 py-[var(--puerta-v1)] text-left"
                  >
          ...
          <div className="min-w-0 lg:self-center">
            <div
              data-puerta="caja"
              className="vidrio vidrio-denso resplandor relative p-[var(--puerta-v3)]"
            >
              {/* El sello de rebaja, pegado en la esquina. */}
              {descuento && (
                <div
                  data-puerta="sello"
                  className="sello-descuento absolute -right-3 -top-4 rounded-xl px-3 py-2 text-center sm:-right-5"
                >
              ...
              <div className="mt-[var(--puerta-v2)] flex items-end gap-3">
                ...
                <span className="cifra text-[clamp(1.9rem,5vh,3rem)] font-bold leading-none text-primary">{precio}</span>
              </div>
              ...
              {/* La llave. */}
              <form onSubmit={enviar} className="mt-[var(--puerta-v3)] border-t border-white/12 pt-[var(--puerta-v3)]">
                ...
                {/* El hueco del error se reserva siempre: si el párrafo apareciera
                    y desapareciera, el botón saltaría bajo el cursor. */}
                <p
                  id="error-clave"
                  role="alert"
            ...
            >
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="microgramma text-base-content/70 underline-offset-4 transition-colors hover:text-primary hover:underline"
