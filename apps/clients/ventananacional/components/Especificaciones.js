...
export function EspecificacionesCompletas({ vehiculo, className = "" }) {
  const datos = [
    ...
    { etiqueta: "Origen", valor: vehiculo.origen },
    { etiqueta: "Sala", valor: vehiculo.ubicacion },
    ...
    <dl className={`divide-y divide-white/10 ${className}`}>
      {datos.map((dato) => (
        <div key={dato.etiqueta} className="flex items-baseline justify-between gap-6 py-3.5">
          ...
          <dd className="text-right text-sm font-medium text-base-content/90">{dato.valor}</dd>
        </div>
      ))}
    </dl>
  );
