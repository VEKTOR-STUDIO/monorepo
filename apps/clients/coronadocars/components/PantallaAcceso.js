 ...
 *
 * Aquí manda la firma de Vektor y no la marca de Coronado Carss: quien llega
 * todavía no es cliente de Coronado Carss, es alguien a quien se le está
...
export default function PantallaAcceso({ destino = "/" }) {
  ...
  return (
    <main ref={raiz} className="puerta relative flex h-svh flex-col overflow-hidden bg-base-100">
      ...
      {/* Velo. El escenario de la portada lleva la banda azul ancha justo a la
          altura de la lista de lo que trae la edición. El velo baja el fondo lo
          justo para que la pared y las bandas se sigan adivinando y el texto
          se lea encima. */}
      <div className="absolute inset-0 bg-base-100/60" aria-hidden="true" />
      ...
      <div
        ...
        aria-hidden="true"
        // El halo va en AZUL REY y no en amarillo, aunque el amarillo fuera lo
        // llamativo: esta marca usa el neón para señalar, y un halo de 40 rem
        // teñido de neón baña la pantalla entera con el único color que aquí
        // significa algo. En azul es la luz de su recuadro.
        style={{
          background:
            "radial-gradient(circle, color-mix(in oklab, var(--color-azul-claro) 55%, transparent), transparent 62%)",
          filter: "blur(90px)",
        }}
      />
      ...
      {/* ---------------- Cuerpo ----------------
          ...
          El logotipo va en vector (components/MarcaCoronado.js), así que el
          contexto de apilamiento que abre el `z-10` no le afecta. */}
        ...
        <div className="m-auto grid w-full max-w-6xl gap-x-[clamp(1.5rem,4vw,3.5rem)] gap-y-[var(--puerta-v4)] px-5 py-[var(--puerta-v2)] lg:grid-cols-[1.15fr_1fr]">
          ...
          <div className="flex min-w-0 flex-col items-center text-center lg:items-start lg:text-left">
            ...
            <div data-puerta="logo" className="mt-[var(--puerta-v3)] flex flex-col items-center lg:items-start">
              ...
              {/* Su logotipo entero, como en su foto de perfil: el recuadro, la
                  camioneta saliéndose y el lema debajo. El recuadro ya dice el
                  nombre, así que el <h1> lo lleva solo para quien no ve. */}
              <h1 className="mt-[var(--puerta-v2)]">
                <span className="sr-only">{config.appName}</span>
                <LogoChapa alto="calc(var(--puerta-logo) * 1.25)" />
              </h1>
            </div>
            ...
            <p data-puerta="titulo" className="puerta-presentacion mt-[var(--puerta-v3)] max-w-md leading-relaxed text-sm text-base-content/65">
              Tu inventario hecho página web, entero y funcionando: usados y 0 km
              separados, tu página de compra y consignación, y la ficha de tus propios
              posts —la pared, la etiqueta del modelo y las cajas de año, modelo,
              kilometraje y transmisión—. Entra con tu contraseña y recórrela.
            </p>
