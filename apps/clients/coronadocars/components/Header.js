...
// El menú es su bio, puesta en fila: comprar o vender. Ese es el corte del
// negocio y es lo primero que dice su perfil —"¿Comprar o vender tu vehículo?
// Para eso estamos"—, así que la cabecera no lo esconde detrás de un
// desplegable.
const ENLACES = [
  { href: "/vehiculos", texto: "Inventario" },
  { href: "/vehiculos?condicion=usado", texto: "Usados" },
  { href: "/vehiculos?condicion=nuevo", texto: "0 km" },
  { href: "/vender", texto: "Vende tu carro" },
        ...
        <div className="ml-auto hidden items-center gap-6 lg:flex">
          {enlaces.map((enlace) => (
            <Link
              key={enlace.href}
              href={enlace.href}
              className="display-abierto text-xs tracking-widest text-base-content/60 transition-colors hover:text-primary"
            >
              {enlace.texto}
            </Link>
          ))}
          ...
          <div className="flex flex-col">
            {enlaces.map((enlace) => (
              <Link
                key={enlace.href}
                href={enlace.href}
                className="display-abierto border-b border-base-content/8 py-3.5 text-base tracking-wide text-base-content/80 transition-colors hover:text-primary"
              >
                {enlace.texto}
              </Link>
            ))}
